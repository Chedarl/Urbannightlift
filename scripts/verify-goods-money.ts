...
check("total is just the fee, and it is exact",
  (() => { const m = orderMoney({ serviceType: "SMALL_PARCEL", deliveryFeeXaf: 1500, goodsCapXaf: null, goodsActualXaf: null, overCapApprovedXaf: null });
...
const pre = orderMoney({ serviceType: "FOOD_PICKUP", deliveryFeeXaf: 1500, goodsCapXaf: 6000, goodsActualXaf: null, overCapApprovedXaf: null });
...
console.log("\nAfter the receipt");
const under = orderMoney({ serviceType: "FOOD_PICKUP", deliveryFeeXaf: 1500, goodsCapXaf: 6000, goodsActualXaf: 5000, overCapApprovedXaf: null });
...
const over = orderMoney({ serviceType: "FOOD_PICKUP", deliveryFeeXaf: 1500, goodsCapXaf: 6000, goodsActualXaf: 8000, overCapApprovedXaf: null });
check("over the cap flags for approval", over.needsCustomerApproval, true);
check("and says by how much", over.overCapByXaf, 2000);

const approvedOver = orderMoney({ serviceType: "FOOD_PICKUP", deliveryFeeXaf: 1500, goodsCapXaf: 6000, goodsActualXaf: 8000, overCapApprovedXaf: 8000 });
...
const settle = riderSettlementForOrder({
  ...
  goodsAdvancedXaf: 5000, totalDueXaf: 6500,
});
...
check("MoMo order: we owe them their share plus their outlay",
  riderSettlementForOrder({ paymentMethod: "MTN_MOMO", riderPayoutXaf: 900, cashCollectedXaf: null, goodsAdvancedXaf: 5000, totalDueXaf: 6500 }),
  -5900);

check("a pure delivery with no shopping is unchanged",
  riderSettlementForOrder({ paymentMethod: "CASH", riderPayoutXaf: 900, cashCollectedXaf: 1500, goodsAdvancedXaf: 0, totalDueXaf: 1500 }),
  600);

check("a shortfall at the door stays visible as a shortfall",
  riderSettlementForOrder({ paymentMethod: "CASH", riderPayoutXaf: 900, cashCollectedXaf: 6000, goodsAdvancedXaf: 5000, totalDueXaf: 6500 }),
  100);

console.log("\nDegenerate inputs never produce a wrong charge");
check("no fee and no cap is zero, not NaN",
  orderMoney({ serviceType: "FOOD_PICKUP", deliveryFeeXaf: null, goodsCapXaf: null, goodsActualXaf: null, overCapApprovedXaf: null }).totalXaf, 0);
